...
```bash
cd server && uv run pytest ../tests                       # all (68 tests, <1s)
...
- **debugger** subagent (read-only: Read, Grep, Glob, Bash) for runtime errors, console errors, failing requests and stack traces. It diagnoses and proposes the fix; the edit still goes through vue-expert for `.vue` files.
- **backend-api-test** skill when writing or modifying anything under `tests/backend/`.
- **vue-component-analyzer** skill when reviewing, auditing or optimizing `.vue` files for performance or code reuse. Its scanner (`.claude/skills/vue-component-analyzer/scripts/scan_components.py`) lists oversized components, index keys and helpers duplicated across files.
...
**Tasks.** `/api/tasks` (GET, POST, PATCH toggles status, DELETE) backs the Tasks modal in `App.vue`. Task fields are camelCase (`dueDate`) because the client merges API tasks with the four hard-coded mock tasks in `useAuth.js`; API ids start at 1000 so the two sets never collide.

**Endpoints the client calls that do not exist.** `api.js` has methods for `/api/purchase-orders` (POST, GET by backlog item id). `main.py` defines `CreatePurchaseOrderRequest` and reads `purchase_orders.json` (currently `[]`) to flag backlog items, but the route is not implemented and nothing in the UI calls it any more.
